...
All financial data were retrieved from Yahoo Finance via the \texttt{yfinance} Python library (freely available). The complete CSV results database, parameter estimates, visualisations, and estimation code are available at the project repository listed in the Appendix.
...
\section*{Appendix: Repository and Supplementary Materials}

The complete project repository — containing the full model parameter reference document (\texttt{ms\_garch\_full\_models.pdf}, 112 pages), the estimation notebook (\texttt{code/Likelihood\_Approximation\_for\_MS\_GARCH.ipynb}), all CSV results for the 220 experimental cases, and the result visualisations — is publicly available at:

\medskip
\noindent\url{https://github.com/oluwadamilarejoshua/ms-garch-likelihood-approximation}
